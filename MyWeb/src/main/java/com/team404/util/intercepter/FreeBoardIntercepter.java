...
public class FreeBoardIntercepter extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		//1. 등록클릭했을 때, 로그인인터셉터로 연결
		//2. Regist화면에서는 작성자를 세션값을 처리, 작성자를 readonly 
		//3. 수정, 변경, 삭제 클릭시 인터셉터를 실행시킴(단, 화면에서는 writer대한 정보를 반드시 넘겨줘야함)
		
		String writer = request.getParameter("writer"); //화면에서 넘어오는 글쓴이에대한 정보
		...
		if( writer != null && writer.equals(user_id) ) { //작성자가 null이 아니고, 세션과 작성자가 같다면 컨트롤러를 실행
			...
			response.setContentType("text/html"); //응답내용대한 내용
